...
// =========================
        ...
        const cantidadMobile = document.getElementById('cantidad-mobile');
        const montoMobile = document.getElementById('monto-mobile');

        if (cantidadMobile) {
        cantidadMobile.textContent = cantidad;
}

        if (montoMobile) {
        montoMobile.textContent = texto;
}
        }
    ...
    fetch('./proyectos.geojson')
        ...
        .then(data => {
            ...
            window.initialBounds = initialBounds;
            /* =========================
   RESET VIEW
========================= */
...
document.addEventListener('click', e => {
    ...
    if (e.target.id === 'north-button') {

        e.preventDefault();

        if (window.initialBounds) {
            map.fitBounds(window.initialBounds);
        }
    }
});
